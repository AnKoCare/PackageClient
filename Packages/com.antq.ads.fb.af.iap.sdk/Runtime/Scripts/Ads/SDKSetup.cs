...
public partial class SDKSetup : ScriptableObject
{
    ...
    public void Setup()
    {
        // AdsManager lives in Assembly-CSharp (imported as a Sample), so the package
        // assembly cannot reference it directly. Resolve it via reflection in editor-only code.
        System.Type adsManagerType = System.Type.GetType("SDK.AdsManager, Assembly-CSharp");
        MonoBehaviour adsManager = adsManagerType != null
            ? (MonoBehaviour)FindObjectOfType(adsManagerType)
            : null;
        if (adsManager != null)
        {
            adsManagerType
                .GetMethod("UpdateAdsMediationConfig", new[] { typeof(SDKSetup) })
                ?.Invoke(adsManager, new object[] { this });
            EditorUtility.SetDirty(adsManager);
            EditorSceneManager.MarkSceneDirty(adsManager.gameObject.scene);
        }
        else
        {
            SDKDebugLogger.LogError("Please add Manager Prefab to scene (Assets/ABIMaxSDKAds/Prefabs/Manager.prefab)");
        }
        ...
        if (adsManager!= null)
        {
            ...
            {
                // AppLovinSettings is a global-namespace ScriptableObject defined in
                // MaxSdk.IntegrationManager.Editor (under Assets), which the package
                // assembly cannot reference directly. Resolve it via reflection.
                System.Type appLovinSettingsType = System.Type.GetType("AppLovinSettings, MaxSdk.IntegrationManager.Editor")
                    ?? System.Type.GetType("AppLovinSettings, Assembly-CSharp-Editor");
                if (appLovinSettingsType != null)
                {
                    string assetPath = "Assets/MaxSdk/Resources/AppLovinSettings.asset";
                    UnityEngine.Object applovinSettings = AssetDatabase.LoadAssetAtPath(assetPath, appLovinSettingsType);
                    if (applovinSettings != null)
                    {
                        appLovinSettingsType.GetProperty("SdkKey")?.SetValue(applovinSettings, sdkKey_MAX);
